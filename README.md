...
- Top tabs: `Focus`, `Inbox`, `Next`, `Later`, `Deferred`, `Closed`, and `Done`
...
- `w`: mark selected `Focus` item as done for today
- `r`: restore selected item from `Closed Today` or `Completed`
...
8. Move to `Closed Today` and press `r`
