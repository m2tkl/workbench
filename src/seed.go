...
func demoState(now time.Time) State {
	...
	release := NewItem(twoDaysAgo.Add(9*time.Hour), "Prepare release notes for v1.4.0", PlacementNow)
	...
	design := NewIssueItem(yesterday.Add(13*time.Hour), "TUI keyboard map review", PlacementNow)
	...
	refactor := NewItem(now.Add(30*time.Minute), "Split state persistence from UI model", PlacementNext)
	refactor.AddNote(now.Add(45*time.Minute), "Needed before adding imports and review flows.")

	backlog := NewIssueItem(now.Add(2*time.Hour), "Add archive export for old completed tasks", PlacementLater)
	backlog.AddNote(now.Add(3*time.Hour), "Keep only if snapshots become necessary.")

	billing := NewItem(yesterday.Add(8*time.Hour), "Submit monthly billing report", PlacementScheduled)
	billing.SetScheduledFor(yesterday.Add(8*time.Hour), today)

	backup := NewItem(twoDaysAgo.Add(7*time.Hour), "Review local backup integrity", PlacementRecurring)
	backup.SetRecurring(twoDaysAgo.Add(7*time.Hour), 2, twoDaysAgo.Format("2006-01-02"))

	laterSchedule := NewIssueItem(now.Add(4*time.Hour), "Prepare team offsite agenda", PlacementScheduled)
	laterSchedule.SetScheduledFor(now.Add(4*time.Hour), now.AddDate(0, 0, 3).Format("2006-01-02"))

	done := NewItem(twoDaysAgo.Add(7*time.Hour), "Write initial storage model", PlacementLater)
