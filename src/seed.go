...
func demoState(now time.Time) State {
	...
	design := NewIssueStockItem(yesterday.Add(13*time.Hour), "TUI keyboard map review", StageNow)
	...
	backlog := NewIssueStockItem(now.Add(2*time.Hour), "Add archive export for old completed tasks", StageLater)
	...
	laterSchedule := NewIssueScheduledItem(now.Add(4*time.Hour), "Prepare team offsite agenda", now.AddDate(0, 0, 3).Format("2006-01-02"))
