...
func TestWorkbenchIndexShowsSidebarAndMainView(t *testing.T) {
	...
	if !strings.Contains(body, `--sidebar-expanded-width: 300px;`) || !strings.Contains(body, `--pane-header-height: 53px;`) || !strings.Contains(body, `grid-template-columns: var(--sidebar-expanded-width) minmax(0, 1fr);`) || !strings.Contains(body, `.layout[data-sidebar-collapsed="true"]`) || !strings.Contains(body, `.layout[data-sidebar-collapsed="true"][data-sidebar-hovered="true"] .sidebar`) || !strings.Contains(body, `width: min(var(--sidebar-expanded-width), calc(100vw - 32px));`) || !strings.Contains(body, `box-shadow: 0 18px 40px rgba(15, 23, 42, 0.18);`) || !strings.Contains(body, `.content-panel {`) || !strings.Contains(body, `.pane-header {`) || !strings.Contains(body, `.content-panel-body {`) || !strings.Contains(body, `.pane-header .section-label {`) || !strings.Contains(body, `height: 32px;`) || !strings.Contains(body, `justify-content: flex-start;`) || !strings.Contains(body, `min-height: var(--pane-header-height);`) || !strings.Contains(body, `font-size: 14px;`) || !strings.Contains(body, `line-height: 1;`) || !strings.Contains(body, `const sidebarStateKey = "workbench.sidebar.collapsed";`) || !strings.Contains(body, `const sidebarCollapsed = () => layout && layout.dataset.sidebarCollapsed === "true";`) || !strings.Contains(body, `const syncSidebarState = () =>`) || !strings.Contains(body, `const setSidebarCollapsed = (collapsed) =>`) || !strings.Contains(body, `const setSidebarHovered = (hovered) =>`) || !strings.Contains(body, `layout.dataset.sidebarCollapsed = collapsed ? "true" : "false"`) || !strings.Contains(body, `window.localStorage.setItem(sidebarStateKey, collapsed ? "true" : "false");`) || !strings.Contains(body, `const persistedSidebarState = window.localStorage.getItem(sidebarStateKey);`) || !strings.Contains(body, `layout.dataset.sidebarCollapsed = persistedSidebarState;`) || !strings.Contains(body, `layout.dataset.sidebarHovered = sidebarCollapsed() && hovered ? "true" : "false"`) || !strings.Contains(body, `setSidebarCollapsed(!sidebarCollapsed());`) || !strings.Contains(body, `toggleSidebarButton.innerHTML = expanded ? "&#9664;" : "&#9654;"`) || !strings.Contains(body, `sidebar.addEventListener("mouseenter", () => setSidebarHovered(true))`) || !strings.Contains(body, `sidebar.addEventListener("mouseleave", () => setSidebarHovered(false))`) || strings.Contains(body, `.content > .panel::before`) || strings.Contains(body, `grid-template-columns: 1fr;`) || strings.Contains(body, `padding: 10px 8px;`) {
		t.Fatalf("expected aligned main sidebar width and header dimensions: %s", body)
	}
	if !strings.Contains(body, `.content-panel {
      display: flex;
      flex-direction: column;
      min-height: 0;`) || !strings.Contains(body, `.content-panel-body {
      flex: 1 1 auto;
      min-height: 0;
      padding: 16px;
      overflow: auto;`) {
	...
	if !strings.Contains(body, `overflow: hidden;`) || !strings.Contains(body, `padding: 12px 16px;`) || strings.Contains(body, `padding: 12px 16px 48px;`) || !strings.Contains(body, `.sidebar {
